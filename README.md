...
- **Live OCEANS-X ingestion with auto-refresh** — manual triggers + background `usePoll` snapshot every 10 minutes. `max_requests_per_run` is 5000; IMO=0 placeholders are filtered at ingestion and on read so AIS contacts no longer collapse into one ghost vessel.
- **Categorical risk model with human-readable surfaces** — sanctioned, detained, watchlist, high-risk flag, identity conflict, adverse news. Each card shows subject, severity, kind, status, source, last checked, and an Evidence chip.
...
V1 has no auth, no AI, no TimescaleDB, no numeric risk score, and no backend scheduler — auto-refresh is a frontend `usePoll` cadence. Write routes belong under `/api/dev/*` and remain development-only. Per-endpoint MPA OCEANS-X scope can restrict access (port-activity endpoints currently return HTTP 400 for our key); positions / particulars / movements work fine.
...
- `/ports` — Port activity inspector (Due-arrive / Due-depart / All ports).
- `/risk` — Risk feed with semantic kind chips (Sanctioned · Detained · Watchlist · Adverse news · High-risk flag · Identity conflict) and the standard `RiskCard` layout.
- `/news` — RSS-derived stories with linked-subject chips.
