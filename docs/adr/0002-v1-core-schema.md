...
SEAM V2 needs a database contract before ingestion and UI work. The V1 charter requires PostgreSQL/PostGIS, latest-position workflows, traceable source evidence, manual ingestion, and transparent risk flags without TimescaleDB, AI, or numeric scoring.
...
V1 stores only `vessel_positions_latest`; it does not create a historical position archive, Timescale hypertable, scheduler table, user table, or AI output table.
